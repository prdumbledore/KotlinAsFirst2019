...
fun square(notation: String): Square {
    ...
    require((notation.last() - '0') in 1..8)
    val firstChar = notation.first() - 'a' + 1
    require(firstChar in 1..8)
    val secondChar = notation.last() - '0'
    return Square(firstChar, secondChar)
}
...
fun rookTrajectory(start: Square, end: Square): List<Square> {
    ...
    return when {
        ...
        start.column != end.column && start.row == end.row -> listOf(start, Square(end.column, start.row))
        start.column == end.column && start.row != end.row -> listOf(start, Square(start.column, end.row))
        else -> listOf(start)
    }
}
...
fun bishopTrajectory(start: Square, end: Square): List<Square> {
    ...
    return when (bishopMoveNumber(start, end)) {
        ...
        1 -> listOf(start, end)
            ...
            for (i in 1..8) {
                for (j in 1..8) {
                    if ((Square(i, j) == start) || Square(i, j) == end) continue
                    if (bishopMoveNumber(start, Square(i, j)) == 1 && bishopMoveNumber(end, Square(i, j)) == 1) {
                        square = Square(i, j)
                        break
                    }
                }
...
fun kingMoveNumber(start: Square, end: Square): Int {
    ...
    return when {
        start == end -> 0
        columnDifference == rowDifference -> columnDifference
        columnDifference > rowDifference -> columnDifference
        else -> rowDifference
    }
}
